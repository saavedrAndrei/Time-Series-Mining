...
df = ts(df[,2],start = c(2013,2),frequency = 365)
df <- na.omit(df)

## Original Dataset
#Clearly the above chart has an upward trend for tractors sales (and there is also a seasonal component).
x11()
plot(df, xlab='date', ylab = 'open')

x11()
par(mfrow = c(2,1))
acf(ts(df),main='ACF RJF-Open')
pacf(ts(df),main='PACF RJF-Open')


## LOG Dataset - No Effect
#Transform data to make data stationary on variance
x11()
plot(log10(df),ylab='Log open')

x11()
par(mfrow = c(1,1))
acf(ts(log10(df)),main='ACF RJF-Open')
pacf(ts(log10(df)),main='PACF RJF-Open')

## DIFF - LOG
#Difference log transformed data to make data stationary on both mean and variance
x11()
plot(diff(log10(df)),ylab='Log Diff RFJ-Open')

x11()
par(mfrow = c(2,1))
acf(ts(diff(log10(df))),main='ACF RJF-Open')
pacf(ts(diff(log10(df))),main='PACF RJF-Open')
...
#Forecast
fcastmod1MA1MAS <- forecast(fit_test_03, h=24)
X11()
